...
class TestXSDCompliance:
    ...
    def test_session_view_request_compliance(self):
        """Startup all-sessions request (no session_id) must pass XSD."""
        xml = sender.build_session_view_request_xml(correlation_id=str(uuid.uuid4()))
        valid, errors = validate_xml(xml, "schema_session_view_request.xsd")
        assert valid, f"XSD Error in session_view_request: {errors}"

    def test_session_view_request_no_correlation_id_compliance(self):
        """session_view_request without correlation_id must still pass XSD (optional field)."""
        import xml.etree.ElementTree as ET2
        root = ET2.Element("message")
        header = ET2.SubElement(root, "header")
        ET2.SubElement(header, "message_id").text = str(uuid.uuid4())
        ET2.SubElement(header, "timestamp").text = "2026-05-13T08:00:00Z"
        ET2.SubElement(header, "source").text = "kassa"
        ET2.SubElement(header, "type").text = "session_view_request"
        ET2.SubElement(header, "version").text = "2.0"
        ET2.SubElement(root, "body")
        xml_bytes = ET2.tostring(root, encoding="unicode")
        xml_str = '<?xml version="1.0" encoding="UTF-8"?>' + xml_bytes
        valid, errors = validate_xml(xml_str, "schema_session_view_request.xsd")
        assert valid, f"XSD Error in session_view_request (no correlation_id): {errors}"

    def test_session_view_response_ok_compliance(self):
        """Planning's all-sessions response with sessions must pass XSD."""
        req_msg_id = uuid.uuid4()
        xml = (
            '<?xml version="1.0" encoding="UTF-8"?>'
            "<message><header>"
            f"<message_id>{uuid.uuid4()}</message_id>"
        ...
        valid, errors = validate_xml(xml, "schema_session_view_response.xsd")
        assert valid, f"XSD Error in session_view_response (ok): {errors}"

    def test_session_view_response_not_found_compliance(self):
        ...
        valid, errors = validate_xml(xml, "schema_session_view_response.xsd")
        assert valid, f"XSD Error in session_view_response (not_found): {errors}"

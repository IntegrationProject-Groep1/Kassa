...
# Load environment
RABBIT_HOST = os.getenv('RABBIT_HOST', 'integrationproject-2526s2-dag01.westeurope.cloudapp.azure.com')
RABBIT_PORT = 5672
...
def test_tcp_connection(host, port, timeout=5):
    ...
    try:
        ...
        print(f"   ❌ Connection refused (port not accepting connections)")
        return False
    except OSError as e:
...
def test_amqp_handshake(host, port):
    ...
    try:
        ...
        if len(data) > 0:
            ...
            print(f"   ❌ Server accepted connection but sent no data")
            return False
...
def test_pika_connection():
    ...
    try:
        import pika
        print(f"   Attempting pika connection (timeout: 5s)...")
        ...
        print(f"   ✅ Pika connected successfully!")
        conn.close()
        return True
    except Exception as e:

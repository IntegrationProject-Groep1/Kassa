...
| We gebruiken version="2.3" in alle berichten, conform het gecentraliseerd XML/XSD contract v2.3. Versiebeheer is de verantwoordelijkheid van de PM's. Als een veld gewijzigd of toegevoegd wordt, communiceren de PM's dit naar alle betrokken teams zodat iedereen gelijktijdig kan aanpassen. |
...
| Formeel goedgekeurd door PM: consumption_order, payment_registered en alle update-events (profile_update, cancel_registration, payment_status, wallet_balance_update, badge_assigned) mogen formeel gebruikt worden.<br><br>Volledig type-overzicht (v2.3): new_registration, badge_scanned, invoice_request, system_error, consumption_order, payment_registered, profile_update, cancel_registration, payment_status, wallet_balance_update, badge_assigned, refund_processed, user_registered, user_unregistered, wallet_lease_request, wallet_lease_return, user_sessions_request, session_view_request.

Nota: `heartbeat` is **niet** meer een Kassa-berichttype. Heartbeats worden uitsluitend afgehandeld door de project-sidecar van het Monitoring/Infrastructuur-team — de Kassa-applicatie heeft daar geen code voor en mag die ook niet implementeren (zie XML/XSD Contract v2.3, SIDECAR PRINCIPLE).<br><br>Actiepunt: documenteer alle XML payloads in ClickUp zodat andere teams exact weten wat ze van ons kunnen consumeren. |

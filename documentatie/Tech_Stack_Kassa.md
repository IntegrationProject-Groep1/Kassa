...
| **Integratietaal** | Python 3.12 | Specifiek de python:3.12-slim Docker image. Integratiecode staat in aparte Python scripts naast Odoo (sender.py, receiver.py, poller.py, heartbeat.py). |
...
| xml.etree.ElementTree | stdlib | XML payloads bouwen in sender.py en heartbeat.py. |
...
| threading | stdlib | Heartbeat en poller op de achtergrond als daemon-thread. |
| time | stdlib | Pauze van 1 seconde tussen heartbeats en polling interval. |
| collections.deque / OrderedDict | stdlib | Rolling error-window in heartbeat.py (deque); LRU idempotentie-cache in receiver.py (OrderedDict). |
...
| Heartbeats niet bufferen | Heartbeats zijn realtime. Gemiste heartbeats worden nooit opgeslagen of later doorgestuurd. |
